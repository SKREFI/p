...
removeFromACharsInB a b = [ x | x <- a, not $ elem x b]
...
splitStringHelper [] _ = []
splitStringHelper (x : rest) acc = if elem x rsDespartitori then acc : (splitStringHelper rest "") else splitStringHelper rest (acc ++ [x])
